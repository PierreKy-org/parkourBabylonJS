...
export default class Rotator extends Box {
  ...
  onPlayerCollision() {
    ...
    this.scene.player.angle =
      (this.scene.player.mesh.rotationQuaternion.y + 90) % 360;
    this.scene.player.mesh.rotationQuaternion.y = this.scene.player.angle;
    this.scene.camera.alpha = BABYLON.Tools.ToRadians(
      (this.scene.player.mesh.rotationQuaternion.y + 90) % 360
    );
